...
const SAMPLES: Record<string, { input: string; output: string }> = {
  fr: {
    input:
      "La nuit, tous les chats sont gris. C'est une verite ancienne: l'obscurite efface les distinctions entre les choses.",
    output:
      "At night, all cats are gray. It is an old truth: darkness erases the distinctions between things.",
  },
  de: {
    input:
      "Wer mit Ungeheuern kampft, mag zusehn, dass er nicht dabei zum Ungeheuer wird.",
    output:
      "Whoever fights monsters should take care not to become a monster in the process.",
  },
  es: {
    input:
  ...
  jp: {
    input:
      "Ningen wa kangaeru ashi de aru. Uchuu wa kare o oshitsubusu koto ga dekiru.",
    output:
      "Man is but a thinking reed. The universe might crush him, and yet he remains nobler than what destroys him.",
  },
  en: {
    input: "Paste any text above to see the translation in action.",
    output:
      "Your text has been translated with contextual fidelity, preserving the author's tone and intent across languages.",
  },
};
    ...
    >
      <div className="max-w-3xl mx-auto text-center">
        ...
        <RevealWrapper delay={80}>
          <h2
            className="font-heading font-light leading-[1.05] mb-3"
        ...
        <RevealWrapper delay={140}>
          <p className="text-[15px] sm:text-[16px] font-light text-muted-foreground mb-12">
            Paste any text in any language and watch the reading layer adapt.
          </p>
        </RevealWrapper>

        <RevealWrapper delay={200}>
          <div
            className="bg-card rounded-2xl border border-foreground/10 overflow-hidden text-left"
          ...
          >
            <div className="p-6 sm:p-8 border-b border-foreground/10">
              <Textarea
                ...
                className="font-heading text-[17px] leading-[1.7] bg-secondary border-foreground/10 rounded-xl min-h-[120px] resize-none focus-visible:ring-primary focus-visible:border-primary/50 placeholder:text-muted-foreground/60 placeholder:font-sans"
              />
