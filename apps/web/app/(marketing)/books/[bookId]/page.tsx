    ...
    <div className="relative aspect-[2/3] overflow-hidden rounded-2xl border bg-muted">
      {book.cover_url ? (
        ...
        <div className="flex h-full w-full items-end bg-gradient-to-br from-primary/80 to-foreground p-5 text-primary-foreground">
          <div>
            <p className="text-xs uppercase opacity-70">{book.author || "Glintpage"}</p>
          ...
          <section className="min-w-0">
            <div className="mb-3 flex flex-wrap gap-2">
              {book.tags.slice(0, 4).map((tag) => (
                <Link key={tag} href={`/library/category/${slugifyCategory(tag)}`}>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Metric
                ...
                value={formatReadingDuration(book.total_read_seconds, "No reads yet")}
              />
              <Metric
                icon={Sparkles}
              ...
              />
              <Metric icon={Library} label="Source" value={sourceLabel(book.source_provider)} />
            </div>
              ...
              >
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0">
                    <h3 className="line-clamp-2 text-sm font-semibold">{related.title}</h3>
                    <p className="mt-1 truncate text-xs text-muted-foreground">
                      {related.author || "Unknown author"}
                    </p>
                    <p className="mt-3 text-xs text-muted-foreground">
                      {formatReadingDuration(related.total_read_seconds, "No reads yet")}
                    </p>
                  </div>
                </div>
              </Link>
